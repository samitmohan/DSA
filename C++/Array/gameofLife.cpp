...
/*
In Place -> Make combination table, original, new state (acc to rules) and replace them by final state
(Truth Table kinda) 1 turned to 1 = 3, 1 turned to 0 = leave as it is (1) and so on...

Original |    New State   | Final State
  0       |       0         |  0
  1       |       0         |  1
  0       |       1         |  2
  1       |       1         |  3

In the end convert all 0,1,2,3 (final states) to new states and return the 2d array.
This get's it done in O(1) space instead of O(n*m) space, TC is still O(8 * n * m) = O(n * m)
*/

/*
Notes :
        countNeighbours()
        // start with top left corner (r-1,r+2)
        // go through the 9 positions, from top left neighbour to bottom right neighbour (c-1,c+2)
        // skipping position out of bounds.
        // and in all other 8, check if value is 1 or 3 (1 -> originally 1), (3 -> originally 1) [LIVING]
        // if living, neighbour++
        // return neighbour.

        main()
        // For every single cell count the number of neighbour (for every posn)
        // Check the 8 directions.
        // Follow rules.
        // Last step -> Go through final list again and change output state to new state (0 -> 0. 1 -> 0, 2 -> 1, 3 -> 1)
*/
